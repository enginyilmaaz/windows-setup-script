...
| CLI Aliases | Global `.cmd` commands in `%USERPROFILE%\apps\aliases\` (added to PATH, so they work from any shell): `cckimi`/`ccglm` (Claude Code on the Kimi / Z.AI-GLM backends) + `cckimi-token`/`ccglm-token` (save the API key) |
